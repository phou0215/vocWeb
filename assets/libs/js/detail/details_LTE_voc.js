...
(function(window, document, $, undefined) {
        ...
        if ($('#chartjs_line1').length) {
              ...
              var jsonData = null;
              $.ajax({
                                          url:"/voc/chart/phpdata/view/view_LTEVoc.php",
                                          type:"post",
                                          dataType:"json",
        ...
        while(i < 4){
            if(i < item_size){
                ...
                //h4 tag에 최근 값 넣기
                $(list_group[i]+" h4").text(jsonData.values[i][0][idx_last]+"건");
                // 전주 비교 수치에서 양수/음수/보합 각각 Red/Blue/Green 입력
                if (jsonData.values[i][3][0] < 0){
                  $(list_group[i]+" h4").attr('class', 'mb-0 text-primary');
                }else if(jsonData.values[i][3][0] > 0){
                ...
                while(j < 3){
                  //증감 수
                  var in_data = jsonData.values[i][2][j];
                  $(item_group[i][j]).text("  "+in_data+" ");
                  //증감율
                  var in_per = jsonData.values[i][3][j];
