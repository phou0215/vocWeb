<?php
    ...
    while($i < count($index_base)){
      ...
      //recent 5G VOC 개수
      $sql = 'SELECT COUNT(*) AS cnt FROM voc_tot_data WHERE regiDate = "'.$recent.'" AND netHead LIKE "'.$index_base[$i].'%" AND netMethod2="5G"';
      $result = mysqli_query($conn, $sql);
      $row = mysqli_fetch_assoc($result);
      $temp_5G_count = $row['cnt'];

      //recent LTE VOC 개수
      $sql = 'SELECT COUNT(*) AS cnt FROM voc_tot_data WHERE regiDate = "'.$recent.'"  AND netHead LIKE "'.$index_base[$i].'%" AND netMethod2="LTE"';
      $result = mysqli_query($conn, $sql);
      $row = mysqli_fetch_assoc($result);
      ...
      if ($avg_type == "1"){
        ...
        //비교 5G VOC 개수
        $sql = 'SELECT COUNT(*) AS cnt FROM voc_tot_data WHERE regiDate = "'.$oneWeek.'" AND netHead LIKE "'.$index_base[$i].'%" AND netMethod2="5G"';
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_assoc($result);
        $temp_5G_pre_count = $row['cnt'];

        //비교 5G VOC 개수
        $sql = 'SELECT COUNT(*) AS cnt FROM voc_tot_data WHERE regiDate = "'.$oneWeek.'" AND netHead LIKE "'.$index_base[$i].'%" AND netMethod2="LTE"';
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_assoc($result);
      ...
      //비교 선택이 한달 평일 평균일 경우
        ...
        //비교 5G VOC 개수
        $sql = 'SELECT COUNT(*) AS cnt FROM voc_tot_data WHERE regiDate IN ('.$holidayString.') AND netHead LIKE "'.$index_base[$i].'%" AND netMethod2="5G"';
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_assoc($result);
        $temp_5G_pre_count = round($row['cnt']/count($index_ex_holiday));

        //비교 5G VOC 개수
        $sql = 'SELECT COUNT(*) AS cnt FROM voc_tot_data WHERE regiDate IN ('.$holidayString.') AND netHead LIKE "'.$index_base[$i].'%" AND netMethod2="LTE"';
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_assoc($result);
      ...
      //비교 선택이 3주 동일 평균일 경우
        ...
        //비교 5G VOC 개수
        $sql = 'SELECT COUNT(*) AS cnt FROM voc_tot_data WHERE regiDate IN ('.$threeWeekString.') AND netHead LIKE "'.$index_base[$i].'%" AND netMethod2="5G"';
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_assoc($result);
        $temp_5G_pre_count = round($row['cnt']/count($index_three_week));

        //비교 5G VOC 개수
        $sql = 'SELECT COUNT(*) AS cnt FROM voc_tot_data WHERE regiDate IN ('.$threeWeekString.') AND netHead LIKE "'.$index_base[$i].'%" AND netMethod2="LTE"';
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_assoc($result);
