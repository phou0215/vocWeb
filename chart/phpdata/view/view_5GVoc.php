<?php
    ...
    if(in_array("total_focus", $index_models)){
      $result = mysqli_query($conn, 'SELECT model FROM '.$model_type.' WHERE focusOn = "1" AND cellType = "5G"');
      $index_focus = array();
     ...
     if(in_array("total_hole", $index_models)){
      $result = mysqli_query($conn, 'SELECT model FROM '.$model_type.' WHERE flag = "1" AND cellType = "5G"');
      $index_total = array();
      while($row = mysqli_fetch_assoc($result)){
