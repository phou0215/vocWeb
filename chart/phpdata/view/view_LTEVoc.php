<?php
    ...
    if(in_array("total_focus", $index_models)){
      $result = mysqli_query($conn, 'SELECT model FROM '.$model_type.' WHERE focusOn = "1" AND cellType = "LTE"');
      $index_focus = array();
     ...
     if(in_array("total_hole", $index_models)){
      $result = mysqli_query($conn, 'SELECT model FROM '.$model_type.' WHERE flag = "1" AND cellType = "LTE"');
      $index_total = array();
      while($row = mysqli_fetch_assoc($result)){
    ...
    while($i < count($index_models)){
      ...
      if($index_models[$i] == 'total_focus'){
        ...
        //전주 당일 데이터
        array_push($temp_avg_array, $temp_array[count($temp_array)-8]);
        $temp_per = get_compared($temp_array[count($temp_array)-8], $temp_array[count($temp_array)-1]);
        ...
        while($j < count($index_ex_holiday)){
          if($temp_array[$index_ex_holiday[$j]] == 0){
            $temp_size = $temp_size - 1;
          }else{
            $temp_data = $temp_data + $temp_array[$index_ex_holiday[$j]];
          }
          $j++;
        }
        $temp_avg = get_avg($temp_data, $temp_size);
        array_push($temp_avg_array, $temp_avg);
        $temp_per = get_compared($temp_avg, $temp_array[count($temp_array)-1]);
        array_push($temp_per_array, $temp_per);
        ...
        while($j < count($index_three_week)){
          if($temp_array[$index_three_week[$j]] == 0){
            $temp_size = $temp_size - 1;
          }else{
            $temp_data = $temp_data + $temp_array[$index_three_week[$j]];
          }
          $j++;
        }
      ...
      array_push($values, [$temp_array, $temp_subs_array, $temp_avg_array, $temp_per_array]);
      $i++;
    }
